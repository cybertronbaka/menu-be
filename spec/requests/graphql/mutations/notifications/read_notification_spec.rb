...
describe 'Mutations: readNotification' do
  ...
  let!(:valid_mutation) do
    <<~GQL
      mutation {
        readNotification(input: {id: #{notification.id}}) {
            id
            notificationType
            body
            link
  ...
  context 'Success' do
    it 'Edits an Item' do
      ...
      expect(parsed.dig(:data, :readNotification)).to_not be_nil
      expect(parsed.dig(:data, :readNotification, :read)).to eq(true)
    end
  end
